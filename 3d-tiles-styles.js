...
<div class="height-46" id="wrapper">
  ...
  <form id="data-form">
        ...
        <div class="filter-group" id="filter-group-0">
          <div class="filter-item-wrapper">
            <div class="form-group-inline filter-item" id="filter-item-0">
              <!-- <input type="text" class="form-input filter-item-attribute" name="filter-item[0][0][attribute]"
                id="filter-item-attribute-0-0"> -->
              <select id="filter-item-attribute-0-0" class="form-input filter-item-attribute custom-select-options"
                name="filter-item[0][0][attribute]">

              </select>
              <div class="form-group">
                <select id="filter-item-operator-0-0" class="filter-item-operator operator"
    ...
    <div class="condition-wrapper" id="condition-wrapper">
      <div class="condition-item" id="condition-item-0">
        <div class="condition-item-fs">
          ...
          <div class="form-group">
            <select id="condition-item-attr-0" class="form-input condition-item-attr "
              name="condition-item[0][0][attr]">
              <!-- <option value="_height">_height</option>
        ...
        <div class="condition-item-sc">
          ...
          <span class="filter-item-del-btn">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M13.4998 3.5L2.49976 3.50001" stroke="#595959" stroke-linecap="round" stroke-linejoin="round" />
              <path d="M6.5 6.5V10.5" stroke="#595959" stroke-linecap="round" stroke-linejoin="round" />
              <path d="M9.5 6.5V10.5" stroke="#595959" stroke-linecap="round" stroke-linejoin="round" />
              <path
                d="M12.5 3.5V13C12.5 13.1326 12.4473 13.2598 12.3536 13.3536C12.2598 13.4473 12.1326 13.5 12 13.5H4C3.86739 13.5 3.74021 13.4473 3.64645 13.3536C3.55268 13.2598 3.5 13.1326 3.5 13V3.5"
    ...
    <label for="add-condition">
      <input type="button" class="blue-btn" id="add-condition" value="条件の追加" />
    </label>
...
<div class="filter-group" id="filter-group">
  <div class="filter-item-wrapper">
    <div class="form-group-inline filter-item" id="filter-item">
      ...
      <div class="form-group">
        <select id="filter-item-attribute" class="filter-item-attribute custom-select-options">
          <!-- <option value="_height">_height</option>
          <option value="_x">_x</option>
          <option value="_y">_y</option> -->
        </select>
      ...
      <div class="form-group">
        <select id="filter-item-operator" class="filter-item-operator" name="filter-item-operator">
          <option value=">=">以上</option>
          <option value="<=">
            以下 </option>
          <option value="===">等しい</option>
...
<div class="condition-item" id="condition-item">
  <div class="condition-item-fs">
    ...
    <select id="condition-item-attr" class="condition-item-attr custom-select-options"
      name="condition-item[temp][attr]">
    </select>
    <div class="form-group">
      <select class="condition-item-operator" name="condition-item[temp][operator]" id="condition-item-operator">
        <option value=">=">以上</option>
...
<div class="filter-item-combine-operator-wrapper" id="filter-item-combine-operator-wrapper">
  <div class="form-group">
    <select id="filter-item-combine-operator" class="filter-item-combine-operator" name="filter-item-combine-operator">
...
<body>
  ...
  <script>
    ...
    $("#filter-item-attribute-0-0").select2({
      allowClear: false,
      minimumResultsForSearch: 1,
    });

    $("#condition-item-attr-0").select2({
      allowClear: false,
      minimumResultsForSearch: 1,
    });
    ...
    function fetchData(url) {
      ...
      return fetch(
        ...
        if (!r) return;
        fetchedData = r.properties;
        return fetchedData;
      });
    }

    function main() {
      let tilesetAll = allLayers?.findAll(tileset => tileset.type === "tileset" && tileset.property.default !== undefined)
      const tilesetUrl = tilesetAll.filter(tileset => tileset.property.default.sourceType === "url" || tileset.property.default.sourceType === undefined)
      const urlArrLinks = [];
      tilesetUrl.map(tu => urlArrLinks.push(tu.property.default.tileset));
      let dataObjArray = [];
      let dataAttrArray = [];

      return Promise.all(urlArrLinks.map(url => fetchData2(url))).then(data => {
        dataObjArray = data;

        //handle data array to be attribute array
        dataObjArray.map(doa => dataAttrArray.push(Object.keys(doa)))
        const parentArray = dataAttrArray.reduce((acc, val) => acc.concat(val), []);

        const uniqueArray = [...new Set(parentArray)];
        //end handle data array to be attribute array
        let osmAttrr = [
          "cesium#color",
          "elementType",
          "elementId",
        ...
        //get the tile attributes based on the url, osm or both
        ...
        if (allEqual(tilesetSrcTypeArr)) {
          ...
          tilesetOSM = null
          attrArr = [...uniqueArray, ...osmAttrr]
        }

        //end get the tile attributes
        addOptionFilter(attrArr);
        addOptionCondition(attrArr)
        createFilterGroup(attrArr);
        handleOnclickEvents(attrArr);
        createConditionItem(attrArr);
        deleteConditionItem(attrArr);
        // const weather = getForecastValue(apiData);
        // updateChart(weather)
        $('#filter-item-attribute-0-0').val("null").trigger('change');
        $('#condition-item-attr-0').val("null").trigger('change');

      });

    }

    function addOptionFilter(arr) {
      let filterItemAttr = document.querySelector(
      ...
      for (var i = 0; i < arr.length; i++) {
        ...
        filterItemAttr.add(option);
      }
    }

    function addOptionCondition(arr) {
      let consitionItemAttr = document.querySelector(
        ".condition-item-attr"
      );
      for (var i = 0; i < arr.length; i++) {
        const option = document.createElement("option");
    ...
    window.addEventListener("message", function (e) {
      ...
      //find all the tileset available in re-earth
      allLayers = e.source.reearth.layers;
      tileset = allLayers.findAll(tileset => tileset.type === "tileset")
      tilesetArr = [];
      for (let i = 0; i < tileset.length; i++) {
        tilesetArr.push(tileset[i])
      }
      ...
      if (property?.hasOwnProperty("default") && !property.default.enable_tag_filter) {
        tagFilterTiles = [];
        selectedTile = tilesetArr;
      } else {
        if (property?.hasOwnProperty("tiles_tag_filter") && property.tiles_tag_filter.length > 0) {
          let tagMap = []
          property.tiles_tag_filter.map((tag, id) => {
            tagMap.push(tag.tiles_tag_name)
          })
          tagFilterTiles = tagMap;
        } else {
          tagFilterTiles = [];
        }
      }

      //get selected tile to apply styles
      if (tagFilterTiles.length == 0 && !property.default?.enable_tag_filter) {
        selectedTile = tilesetArr;
      } else if (property.default.enable_tag_filter && tagFilterTiles.length == 0) {
        selectedTile = [];
      } else {
        selectedTile = getTileByTagArray(tilesetArr, tagFilterTiles);
      }

      //array for check tag change, length is always <= 2
      tagChangeArr.push(tagFilterTiles);
      while (tagChangeArr.length > 2) {
        tagChangeArr.shift()
      }
    ...
    //get 3d by array tag <AND>
    // function getTileByTagArray(tileArray, tagArray){
    //   let result = [];
    //   let temp = tileArray;
    //   for(let i=0; i<tagArray.length; i++) {
    //     temp = getTileByTag(temp,tagArray[i])
    //   }
    //   result = temp;
    //   return result;
    // }

    //get 3d by array tag <OR>
    function getTileByTagArray(tileArray, tagArray) {
      let result = [];
      let temp;
      for (let i = 0; i < tagArray.length; i++) {
        temp = getTileByTag(tileArray, tagArray[i])
        result.push(temp);
      }

      arrayR = result.map(function (x) {
        return x[0];
      });
      data = arrayR.filter(function (element) {
    ...
    // Get 3dtiles by selected tag
    function getTileByTag(tileArr, nameTag) {
      let temp = [];
      tileArr.map((tile) => {
        if (tile.tags != undefined) {
          tile.tags.map(tagElm => {
    ...
    function createFilterItem(target, arr = []) {
    ...
    function generateNewFilterItemID(arr = []) {
      let addFilterItemBtns = document.querySelectorAll(
        ".filter-group-wrapper .add-filter-item"
      );
      ...
      addFilterItemBtns.forEach(function (item, ind) {
        ...
        filterItems.forEach(function (filterItem, ind2) {
          ...
          $("#filter-item-attribute-" + ind + "-" + ind2).select2({
            allowClear: false,
            minimumResultsForSearch: 1,
          });
    ...
    function createFilterGroup(arr = []) {
    ...
    function deleteFilterItem(delBtnNode, arr = []) {
      let filterGroup = delBtnNode.closest(".filter-group");
      let filterGroupWrapper = document.getElementById(
        "filter-group-wrapper"
      );
    ...
    function createConditionItem(arr = []) {
      ...
      addConditionBtn.addEventListener("click", function () {
        ...
        spanText.innerHTML = conditionLength > 0 ? "追加 条件" : "条件"
    ...
    // Handle deleting filter item
    function deleteConditionItem(arr = []) {
    ...
    // Re-generating Filter Group ID after creating filter group
    function generateConditionID(arr) {
      ...
      addConditions.forEach(function (condition, ind) {
        ...
        if (!conditionAttr.value) {
          for (var i = 0; i < arr.length; i++) {
            if (conditionAttr.options.length == 0) {
              $("#condition-item-attr-" + ind).val(null).trigger("change");
            }

            const option = document.createElement("option");
            option.value = arr[i];
            option.innerHTML = arr[i];
            conditionAttr.add(option);
          }
        }
        ...
        $("#condition-item-attr-" + ind).select2({
          allowClear: false,
          minimumResultsForSearch: 1,
        });
    ...
    const summarizeCondition = (summarizeParam = REMOVE_STYLE_PARAMS_NUM) => {
              ...
              if (filterArr[i][j]["value"]) {
                if (isNumeric(filterArr[i][j]["value"])) {
                  ...
                  if (filterArr[i][j]["operator"] === "===" || filterArr[i][j]["operator"] === "!==") {
                    ...
                    // alert("Filter operator must be 等しい or 等しくない with " + filterArr[i][j]["value"] + ". Please try again!")
                    console.error("Filter operator must be 等しい or 等しくない with " + filterArr[i][j]["value"])
                  }
                }
              } else {
          ...
          if (colorArr[i]["value"]) {
            ...
            if (isNumeric(colorArr[i]["value"])) {
              ...
              if (colorArr[i]["operator"] === "===" || colorArr[i]["operator"] === "!==") {
                ...
                // alert("Color operator must be 等しい or 等しくない with " + colorArr[i]["value"] + ". Please try again!")
                console.error("Color operator must be 等しい or 等しくない with " + colorArr[i]["value"])
              }
            }

            //end handle color === "undefined"
            //end color have value
      ...
      if (removeStatus !== REMOVE_STYLE_PARAMS_NUM) {
        ...
        //is tag array changed to previous tag array
        if (tagChangeArr.length > 1) {
          for (let i = 0; i < tagChangeArr.length; i++) {
            if (tagChangeArr[i + 1] !== undefined) {
              isChangedTag = compareArrays(tagChangeArr[i + 1], tagChangeArr[i])
            } else {
              isChangedTag = true;
            }
          }
        ...
        let emptyData = removeData =
          "data:text/json;charset=utf8," +
          encodeURIComponent(JSON.stringify(""));
        //post jsondata to re-earth
        if (isChangedTag) {
          parent.postMessage({ styleJsonData, emptyData, tilesetArr, selectedTile }, "*");
        } else {
          parent.postMessage({ styleJsonData, selectedTile }, "*");
        }
        ...
        styleJsonData =
          ...
          encodeURIComponent(JSON.stringify(""));
        //post jsondata to re-earth
        parent.postMessage({ removeData, selectedTile }, "*");
      }
    };
